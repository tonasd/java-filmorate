...
public class ReviewDaoImpl implements ReviewDao {
    ...
    public Review update(Review review) throws ReviewNotFoundException {
        try {
            String sql = "UPDATE REVIEWS SET " +
                    "IS_POSITIVE = ?, " +
                    "CONTENT = ? " +
                    "WHERE REVIEW_ID = ? AND IS_DELETED IS FALSE";
    ...
    public Review getReviewById(long reviewId) throws ReviewNotFoundException {
        try {
            String sql = "SELECT * FROM REVIEWS WHERE REVIEW_ID = ? AND is_deleted IS FALSE";
    ...
    public List<Review> getReviewsByFilmId(long filmId, int count) {
        String sql1 = "SELECT * FROM REVIEWS WHERE is_deleted IS FALSE ORDER BY USEFUL DESC LIMIT ?";
